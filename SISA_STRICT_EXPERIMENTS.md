...
最终机器可读证据集中存放在 Git 忽略的 `artifacts/sisa_native_strict/` 目录：

- `runs.csv`：32 个逻辑任务的 job/task ID、GPU 型号、协议、完成标记、指标有限性、
  错误数和完成状态；
- `metrics.csv`：全部 152 行 baseline/SISA 原始指标；
- `summary.md`：全部 76 组配对 AUC/Logloss 及其差值。
